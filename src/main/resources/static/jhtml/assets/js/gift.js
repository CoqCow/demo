...
function retrieveData(sSource, aoData, fnCallback) {
    ...
    var regionId = $('#input_region').val().trim();
    if (regionId == -1) {
        regionId = null;
    }
    $.ajax({
        ...
        success: function (result) {
            ...
            for (var key in result.goodsRegionMap) {
                if (regionId == key) {
                    var str = "<option value=\"" + key + "\" selected>" + result.goodsRegionMap[key] + "</option>";
                    $("#input_region").append(str);
                } else {
                    var str = "<option value=\"" + key + "\">" + result.goodsRegionMap[key] + "</option>";
                    $("#input_region").append(str);
                }
...
function uploadBtn() {
    ...
    $.ajax({
        ...
        success: function (data) {
            alert(data.message);
            if (data.data > 0) {
                window.location.href = 'http://cshop.jd.com/recommend/errorlist';
            }
            table.fnDraw();
        }
    })
